<script lang="ts">
	...
	import { setupHighlighter, clearSelection, highlightByID, getFirstModelName } from '$lib/utils/highlighter';
	import { calculateCompassAxes, resetView, recenterView, fitCameraToModel, type CameraState } from '$lib/utils/camera';
	import type { FragmentsManager, FragmentGroup, IFCLoader, SelectedElement, CompassAxes } from '$lib/types/viewer';
	...
	async function setupFragments() {
		...
		const list = fragments.list as unknown as { onItemSet?: { add?: (fn: (e: { value: FragmentGroup }) => void) => void } };
		list.onItemSet?.add?.((event) => {
			...
			(fragments as unknown as { core?: { update?: (force: boolean) => void } }).core?.update?.(true);
		});
...
<div class="relative flex h-full w-full flex-col">
	<!-- Header -->
	<div class="z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 shadow-sm">
	...
	<div class="relative flex flex-1 overflow-hidden">
		...
		>
			...
			{#if isLoading}
				<div class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-4 bg-white/90 backdrop-blur-sm">
					<div class="h-16 w-16 animate-spin rounded-full border-4 border-gray-200 border-t-blue-500"></div>
			...
			{#if hasModel}
				...
				<div class="absolute right-4 bottom-4 z-10 flex flex-col items-center overflow-hidden rounded-xl border border-gray-200 bg-white/90 shadow-lg backdrop-blur-sm">
					<div class="p-2">
						<svg class="h-28 w-28" viewBox="-44 -44 88 88">
							...
							{#each axes as { axis, color, data }}
								...
								<line x1="1" y1="1" x2={data.x + 1} y2={data.y + 1} stroke="rgba(0,0,0,0.15)" stroke-width={strokeWidth + 1} stroke-linecap="round" />
								<line x1="0" y1="0" x2={data.x} y2={data.y} stroke={color} stroke-width={strokeWidth} stroke-linecap="round" {opacity} />
								...
								<text x={data.x} y={data.y} text-anchor="middle" dominant-baseline="central" fill="white" font-size={9 + (data.z + 1) * 0.5} font-weight="bold">{axis.toUpperCase()}</text>
							{/each}
					...
					<div class="flex w-full">
						<button onclick={handleRecenterView} class="flex flex-1 cursor-pointer items-center justify-center border-r border-gray-200 px-3 py-2.5 transition-colors hover:bg-gray-100" title="Recenter">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
								<path d="M2 8V2h6" /><path d="M22 8V2h-6" /><path d="M2 16v6h6" /><path d="M22 16v6h-6" />
								<circle cx="12" cy="12" r="2" fill="currentColor" />
							</svg>
						</button>
						<button onclick={handleResetView} class="flex flex-1 cursor-pointer items-center justify-center px-3 py-2.5 transition-colors hover:bg-gray-100" title="Reset View">
				...
				<div class="animate-fade-in absolute top-4 right-4 z-30 max-w-md">
					<div class="rounded-lg border border-red-200 bg-white p-4 shadow-lg">
						<div class="flex items-start gap-3">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
							...
							<button onclick={() => (error = null)} class="flex-shrink-0 cursor-pointer text-red-400 transition-colors hover:text-red-600" aria-label="Close">
